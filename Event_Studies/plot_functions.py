...
def plot_price_X_period (X_asset, X_Market, Y_date, asset):
    """
    [Summary]:
    [Args]:
    [Returns]:
    ...
    plt.plot(Y_date, X_Market/100, color='#2D708EFF', label='^NDX / 100', linestyle='solid')
...
def plot_return_X_period (X_asset, X_Market, Y_date, asset):
    """
    [Summary]:
    [Args]:
    [Returns]:
    ...
    axs[0].plot(Y_date, X_Market, color='#39568CFF', label='^NDX', linestyle='solid')
    axs[1] = sns.distplot(X_Market, kde=True, color='#39568CFF', vertical=True)
...
def plot_market_X_asset(Y_asset, X_Market, asset):
    """
    [Summary]:
    [Args]:
    [Returns]:
    """  
    plt.figure(figsize=(10, 7))
    sns.regplot(x=X_Market, y=Y_asset)
